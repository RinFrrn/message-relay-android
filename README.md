...
- 应用独立规则（设置 → 软件选择 → App 规则编辑页，或页内「批量管理规则」）；
- 自定义消息模板（设置 → 消息模板 → 自定义模板库）；
- 多渠道推送、失败自动重试（设置 → 高级设置）。
...
模板只有一个一级入口：**设置 → 消息模板**（全局）——页内选择全局默认模板（新建 App 规则时使用的默认），并可进入「自定义模板库」新建、编辑和删除模板；**规则编辑页**（按应用）为单个 App 选择模板。入口处有「全局 / 按应用」标记，所有入口看到的模板选项保持一致。
