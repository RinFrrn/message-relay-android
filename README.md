...
模板有两个入口，作用域不同（入口处有「全局 / 按应用」标记）：**高级设置 → 自定义消息模板**（全局）负责新建和删除模板；**规则编辑页**（按应用）统一为单个 App 选择模板。**设置 → 消息模板**（全局）是新建 App 规则时的默认模板。所有入口看到的模板选项保持一致。
...
记录保留时间在「高级设置 → 记录与隐私」里设置。同一处还提供隐私显示模式：完整显示、隐藏正文（记录详情里隐藏正文）、隐藏号码（7 位以上数字串保留后 4 位打码）。
